...
# Virtual Machine Scale Set for Business Tier
resource "azurerm_linux_virtual_machine_scale_set" "business_vmss" {
  ...
  network_interface {
    ...
    primary = true
    ip_configuration {
      name      = "business-tier-ip"
      primary   = true
      subnet_id = azurerm_subnet.business.id
      load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.business_pool.id]
    }
  }
}
